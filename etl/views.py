...
class ETLRunView(View):
    ...
    def post(self, request):
        try:
            ...
            return JsonResponse(
                {"success": True, "job_id": job.id, "message": "ETL process started"}
            )
...
def run_etl_tests(request):
    ...
    summary = f"""
    <h2>Test Results</h2>
    <p>Run: {result.testsRun}</p>
    <p>Failures: {len(result.failures)}</p>
    <p>Errors: {len(result.errors)}</p>
    <pre>{output}</pre>
    """

    return HttpResponse(summary, content_type="text/html")
